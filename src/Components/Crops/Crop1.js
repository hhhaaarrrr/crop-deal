...
editCrop(id){
    this.props.navigate(`/updatecrop/${id}`)
}

deleteCrop(id){
    cropService.deleteCrop(id).then(res =>{
        this.setState({allCrops: this.state.allCrops.filter(crops => crops.id !== id)});
    })
}
    ...
    render() {
        return (
            <div>
                ...
                <h1 className='text-center'> Crop List </h1>
                <div className='row'>
                        ...
                        <tbody>
                            ...
                            {this.state.allCrops.map(crops => 
                                <tr key={crops.id}>
                                    ...
                                    <button onClick={ () => this.editCrop(crops.id)} className="btn btn-info">Update </button>
                                    <button style={{marginLeft: "10px"}} onClick={ () => this.deleteCrop(crops.id)} className="btn btn-danger">Delete </button>
                    ...
                    <Link to="/addcrops"> <button className='btn btn-primary'>Add Crop</button> </Link> 
...
export default function WithNavigate(props) {
    ...
    return <Crop1 {...props} navigate={navigate} />
}